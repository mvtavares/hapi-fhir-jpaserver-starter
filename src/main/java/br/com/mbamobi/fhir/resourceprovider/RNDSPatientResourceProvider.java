...
public class RNDSPatientResourceProvider implements IResourceProvider {
	 ...
	 */
	private Map<Long, Deque<RNDSPatient>> myIdToPatientVersions = new HashMap<Long, Deque<RNDSPatient>>();
	...
	/**
		 ...
		 */
		public RNDSPatientResourceProvider() {
			long resourceId = myNextId++;
			
			RNDSPatient patient = new RNDSPatient();
			...
			LinkedList<RNDSPatient> list = new LinkedList<RNDSPatient>();
			list.add(patient);
	 ...
	 */
	private void addNewVersion(RNDSPatient thePatient, Long theId) {
		InstantDt publishedDate;
		if (!myIdToPatientVersions.containsKey(theId)) {
			myIdToPatientVersions.put(theId, new LinkedList<RNDSPatient>());
			publishedDate = InstantDt.withCurrentTime();
		}

		Deque<RNDSPatient> existingVersions = myIdToPatientVersions.get(theId);
	 ...
	 */
	@Create()
	public MethodOutcome createPatient(@ResourceParam RNDSPatient thePatient) {
	 ...
	 */
	@Search()
	public List<RNDSPatient> findPatientsByName(@RequiredParam(name = Patient.SP_FAMILY) String theFamilyName) {
		LinkedList<RNDSPatient> retVal = new LinkedList<RNDSPatient>();

		/*
		 * Look for all patients matching the name
		 */
		for (Deque<RNDSPatient> nextPatientList : myIdToPatientVersions.values()) {
			RNDSPatient nextPatient = nextPatientList.getLast();
	...
	@Search
	public List<RNDSPatient> findPatientsUsingArbitraryCtriteria() {
		LinkedList<RNDSPatient> retVal = new LinkedList<RNDSPatient>();
	 ...
	 */
	@Override
	public Class<RNDSPatient> getResourceType() {
		return RNDSPatient.class;
	}
	 ...
	 */
	@Read(version = true)
	public RNDSPatient readPatient(@IdParam IdType theId) {
		Deque<RNDSPatient> retVal;
		try {
		...
		if (theId.hasVersionIdPart() == false) {
			...
			for (RNDSPatient nextVersion : retVal) {
	 ...
	 */
	@Update()
	public MethodOutcome updatePatient(@IdParam IdType theId, @ResourceParam RNDSPatient thePatient) {
		validateResource(thePatient);
	 ...
	 */
	private void validateResource(RNDSPatient thePatient) {
		/*
